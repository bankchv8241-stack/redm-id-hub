...
st.markdown("""
    ...
    button:has(div:contains("บันทึก")):hover, button:has(div:contains("CONFIRM")):hover { border-color: #BD00FF !important; color: #BD00FF !important; box-shadow: 0 0 20px rgba(189, 0, 255, 0.4); }
...
if check_password():
    ...
    if sheet:
        ...
        if not df.empty:
            st.markdown("### 📊 สรุปภาพรวมระบบ")
            c1, c2, c3 = st.columns(3)
            c1.metric("จำนวนไอดีทั้งหมด", len(df))
            c2.metric("เซิร์ฟเวอร์ที่ใช้งาน", len(df['Server_Name'].unique()))
            c3.metric("อัปเดตล่าสุด", df.iloc[-1]['In_Game_Name'])
            st.markdown("---")

        with st.expander("➕ เพิ่มโปรไฟล์ใหม่"):
            with st.form("add_new_form", clear_on_submit=True):
                c1, c2 = st.columns(2)
                with c1:
                    new_id = st.text_input("ไอดีสตรีม (Account Name)")
                    new_email = st.text_input("อีเมลสตรีม (Email)")
                    new_pwd = st.text_input("รหัสผ่าน (Password)")
                with c2:
                    new_hex = st.text_input("Steam Hex")
                    new_ig_name = st.text_input("ชื่อตัวละครในเกม (IC Name)")
                    new_server = st.text_input("ชื่อเซิร์ฟเวอร์ (Server)")
                ...
                if st.form_submit_button("บันทึกข้อมูล"):
                    if new_id and new_ig_name:
                        ...
                        send_discord_log("เพิ่มโปรไฟล์ใหม่", f"**ชื่อ:** {new_ig_name}\n**เซิร์ฟเวอร์:** {new_server}")
            ...
            for idx, row in f_df.iterrows():
                ...
                with st.container(border=True):
                    ...
                    with col_i:
                        ...
                        st.write(f"🌐 **เซิร์ฟเวอร์:** {row['Server_Name']}")
                        if st.checkbox(f"ดูรหัสผ่าน", key=f"pw_{idx}"): st.write(f"🔑 `{row['Password']}`")
                    with col_a:
                        ...
                        if c_d.button("🗑️", key=f"del_{idx}"):
                            ...
                            send_discord_log("ลบโปรไฟล์", f"**ชื่อ:** {row['In_Game_Name']}")
                            st.rerun()
                    ...
                    if st.session_state.get(f"ed_{idx}", False):
                        with st.form(f"fe_{idx}"):
                            u_ig = st.text_input("แก้ไขชื่อ", value=row['In_Game_Name'])
                            u_sv = st.text_input("แก้ไขเซิร์ฟเวอร์", value=row['Server_Name'])
